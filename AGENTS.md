...
4. Do the work. Use `create_task` / `edit_task` to add or update tasks, and `create_subtask` for
   ...
   spec, in a handoff. Record progress with `edit_task` (implementationNotes / finalSummary).
   Finishing a task is `request_merge`'s job — it marks the task **Done** on the board and it stays
   there, in `tasks/`, where everyone can see it. There is no tool that files finished work away off
   the board, and you should not want one.
