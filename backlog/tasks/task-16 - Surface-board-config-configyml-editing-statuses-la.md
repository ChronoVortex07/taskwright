...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 A `taskwright.editBoardConfig` command (palette + a control in the board/settings UI) opens config editing.
- [x] #2 Users can add / rename / reorder / remove statuses; changes propagate to kanban columns without a manual reload.
- [x] #3 Users can add / remove labels and milestones.
- [x] #4 Users can set default_status, definition_of_done, and the git/behavior flags (auto_commit, check_active_branches, active_branch_days, remote_operations, bypass_git_hooks).
- [x] #5 Writes go through a dedicated config writer (new `ConfigWriter` or extend `BacklogWriter`) that round-trips the existing `config.yml` byte-for-byte except for the changed keys — verified against a fixture matching upstream Backlog.md output.
- [x] #6 Invalid edits are rejected (e.g. removing a status still used by tasks warns/blocks; renaming a status offers to migrate affected tasks).
- [x] #7 Unit tests cover the config writer round-trip and validation; behavior aligns with upstream `backlog config` where it exists.
<!-- AC:END -->

## Implementation Notes

<!-- SECTION:NOTES:BEGIN -->
Implemented TASK-16 following TDD:

### New files
- `src/core/ConfigWriter.ts` — Surgical config.yml editor with renderValue, rewriteArrayLine, rewriteScalarLine, applyConfigEdits, validateConfigEdits. Follows the established mergeStatusConfig.ts / categoriesConfig.ts line-by-line replacement pattern. Preserves EOL, all unchanged lines, and never re-serializes YAML.
- `src/test/unit/ConfigWriter.test.ts` — 44 unit tests covering renderValue, array/scalar editing, multi-edit apply, round-trip verification, validation of status-in-use, rename detection, and CRLF preservation.
- `src/webview/components/tasks/ConfigEditor.svelte` — Modal overlay (matching CreateTaskForm.svelte pattern) with 4 tabs: Statuses (reorder/rename/add/remove), Labels (add/remove), Milestones (add/remove), Defaults & Flags (default_status dropdown, definition_of_done checklist, boolean toggles, active_branch_days number input).

### Modified files
- `src/core/types.ts` — Added ConfigEdit interface, configData/configEditResult/openConfigEditor ExtensionMessages, requestConfigData/saveConfigEdits WebviewMessages.
- `src/providers/TasksController.ts` — Added sendConfigData() helper, saveConfigEdits handler (writes config via ConfigWriter, invalidates cache, migrates renamed statuses, refreshes board), requestConfigData handler.
- `src/webview/components/tasks/Tasks.svelte` — Wired ConfigEditor modal, config button (gear icon), message handlers for configData/configEditResult/openConfigEditor.
- `src/extension.ts` — Registered taskwright.editBoardConfig command.
- `package.json` — Added taskwright.editBoardConfig command with gear icon and palette entry.

### Test results
- 97 test files, 1571 tests all passing
