...
| **Taskwright: Set Up Codex Integration (MCP + prompts)**         | Registers the **Taskwright MCP server** in Codex's `~/.codex/config.toml` and installs the four skills as **Codex custom prompts**; offers the shared `AGENTS.md` convention block. | If you dispatch tasks to **Codex** (`taskwright.dispatchAgent: "codex"`). Taskwright offers this automatically when it detects an un-integrated Codex install.                                                                  |
