    ...
    'request_merge',
    {
      ...
      inputSchema: {
        ...
        verifyTimeoutMinutes: z
          ...
          .optional()
          .describe(
            'Optional per-call verify timeout in minutes (e.g. after measuring a long suite). Overrides the repo default (taskwright.mergeVerifyTimeoutMinutes, 20 min if unset); clamped to the repo-level max when one is configured. Non-positive values are ignored. You should rarely need this: verify runs are serialized across concurrent request_merge calls (a shared verify slot), so a suite is no longer slowed by other merges verifying at the same time. Raise it only when a command genuinely aborts with code `verify_timeout` — a `verify_failed` abort means the command went RED, and a bigger timeout cannot fix that.'
          ),
        waitMinutes: z
          .number()
          .optional()
