...
**6. Verify commands are per-task and concrete** (`bun run test -- treeGeometry`, `… -- TasksController`, `bun run test:playwright -- tree-drag`, `bun run test:cdp`, plus the Task 10 full gate). Commits stage only named files. The orchestrator lands the branch — Task 10 stops at "worktree clean, gates green," not `request_merge`.
