...
**6. Verify commands are per-task and concrete** (`bun run test -- <suite>`, `bun run test:playwright -- tree-`, the full `bun run test:playwright`/`bun run test:cdp` at Task 6, the full gate at Task 8). Commits stage only named files. Model tiers: Tasks 1–6, 8 opus (cores/integration/Svelte/judgment); Task 7 haiku (fully-provided SKILL.md — genuine transcription).
