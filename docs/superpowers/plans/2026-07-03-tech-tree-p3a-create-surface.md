...
**6. Verify commands are per-task and concrete** (`bun run test -- createTaskCore`, `… -- TasksController`, `bun run test:playwright -- tree-authoring`, `bun run test:cdp`, plus the full gate in Task 8). Commits stage only named files.
