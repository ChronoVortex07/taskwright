...
- **LF line endings for source:** repo git config is `autocrlf=false`, `eol=lf`; committed blobs are LF. (Windows note: the lint-staged pre-commit hook can flip the whole tree CRLF→LF — if you hit that on Windows, commit with `--no-verify`; do not restructure code around it.)
