...
**6. Verify commands are per-task and concrete** (`bun run test -- <suite>`, `bun run test:playwright -- tree-`, the full `bun run test:playwright`/`bun run test:cdp` at Tasks 1 & 4). Commits stage only named files. Model tiers: Tasks 1 & 4 opus (cross-cutting model change / judgment); Tasks 2 & 3 DeepSeek (self-contained mechanical mirror + verbatim transcription), each with an Opus hard-read-only review.
